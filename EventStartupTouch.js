...
-----
Triggers events with mouse clicks and touches.
While the currently active page is triggered,
you can turn on any switch to branch processing.
When the event ends, the switch will automatically return to OFF.

Default touch movement will be forcibly disabled.

- Script Details (Execute from the "Script" event command)
Temporarily disables touch startup.
$gamePlayer.setTouchStartupDisable();

Re-enables disabled touch startup.
...
@desc The switch number that will be turned ON when the event is triggered by touch
@type switch
@default 0

@param ValidTriggers
@desc This is the trigger that enables touch activation. (0: Confirm button...4: Parallel processing)
@default 0,1,2
*/
