...
@plugindesc Database Conversion Plugin
@author Triacontane
@license MIT License
...
DatabaseConverter.js
Exports and imports a Maker MV database to Excel or other spreadsheet data
(hereinafter referred to as a "sheet file").

You can export and import by executing the specified plugin command
(described below) from Event Test (right-click on the Event Editor and
 select "Test").

--------------------------------------
Export Procedure
--------------------------------------
1. Turn on this plugin from the admin panel.
2. Execute "Save Project." (First time only)
3. Execute the specified plugin command from Event Test.
4. The sheet file will be output to the specified folder.

--------------------------------------
Import Procedure
--------------------------------------
1. Execute "Save Project."
2. Back up the Data folder.
3. Execute the specified plugin command from Event Test.
4. Execute "Open Project" to reopen the project.
Do not save the project when reopening it.

*Please execute plugin commands one at a time.
Multiple commands cannot be executed at once.

--------------------------------------
Procedure for Creating an Original Database (Advanced)
--------------------------------------
1. Turn off the "Load Original Data" parameter.
2. Add original data to the "Target Database" parameter.
3. Export the database.
4. The original data sheet will be added, allowing you to edit it freely.
5. Import the database.
6. Turn on the "Load Original Data" parameter.
7. You can reference the original data from a script.

-------------------------------------
Target Data to Output
--------------------------------------
- Database
Inputs and outputs an existing database. The file name is "Database."
An extension will be added based on the output format.
 (Same applies below.)
Tile sets and common events are not included.
You can also import and export custom data (referenced from scripts).

- Common Events
Inputs and outputs the execution details of common events.
The file name is "CommonEvents."
For parameter details, see the spreadsheet below.
https://docs.google.com/spreadsheets/d/1rOIzDuhLC6IqJPEFciYOmXWL_O7X9-hMValMs7DpWCk/edit#gid=1266374350

・Map Event
Inputs and outputs the execution details of map events.
The file name is "MapXXX".
The "Event ID" and "Event Page" are output as the sheet name.

・Event Test
Outputs the execution details selected in the event test.
The file name is "Test_Event". Output only is supported.

-------------------------------------
Output Format
--------------------------------------
In addition to Excel files, the following formats are supported, including CSV and OpenDocument Spreadsheet.
The following formats can be changed using the plugin parameters.

xlsx: The standard format for Excel 2007 and later.
xlsm: The macro-enabled format for Excel 2007 and later.
xlsb: The binary format for Excel 2007 and later. It offers superior
capacity and speed.
ods: An open file format that is vendor-independent.
fods: An open XML text file format that is vendor-independent.
csv: A comma-separated text file format.
txt: A tab-separated text file format.

-------------------------------------
Output File Details
--------------------------------------
1. If a file with the same name exists at the time of output, it will be
overwritten. (*1)
2. The sheet name contains the data type, so do not edit it.
3. The first line of the output file contains information required for
input/output (property names).
Do not edit this field either.
4. The second line contains the Japanese name of the field. It is ignored
when loading.
5. Array fields (such as features) are output as JSON strings, with some
exceptions. Editing is not recommended.
6. Numerical values and strings can be edited, but be careful not to enter
inconsistent values.
7. Excel formulas will load the results as database values.
8. You are free to customize the formatting, set matrices, and add macros.
9. The database is sorted by the ID column when loaded. Duplicates will
result in an error.

*1 If a file with the same name is open, an error will occur, so please
close the file.
-------------------------------------
Notes
--------------------------------------
Before using this plugin's features, be sure to back up the "data" folder
under your project.

We also distribute a plugin that automatically backs up the contents of
the "data" folder.
Please consider using this plugin as well.
https://raw.githubusercontent.com/munokura/triacontane-MV-plugins/refs/heads/main/BackUpDatabase.js

Please note that we cannot restore damaged projects under any
circumstances.

If you receive an error message when opening the exported Excel file, do
not edit or import it.

This plugin's functionality is only effective when run from the
"Event Test" menu.
It does not affect normal test play or the retail version.
(The function for importing custom data is an exception.)
...
EXPORT_DATABASE
Exports the contents of the database in the specified format.

IMPORT_DATABASE
Imports a sheet file and rewrites the database file.
To reflect the changes in the editor, you must reopen the project.

EXPORT_COMMON_EVENT 2
Exports the execution details of the common event with ID [2].
Multiple IDs can be specified.
If no ID is specified, all common events will be exported.

IMPORT_COMMON_EVENT 3
Imports a sheet file and rewrites the common event with ID [3].
If no ID is specified, all common events will be exported.

EXPORT_MAP_EVENT 6 20
Exports the execution details of the event with ID [20] in the map with ID
[6].
Multiple event IDs can be specified. If omitted, all events will be exported.
The map ID cannot be omitted.

IMPORT_MAP_EVENT 6 20
Loads a sheet file and rewrites the action of the event with ID [20] in
the map with ID [6].
Multiple event IDs can be specified; if omitted, all events will be
rewritten. The map ID cannot be omitted.

-------------------------------------
Library Used
--------------------------------------
The Community Edition of SheetJS is used to analyze Excel data.
http://sheetjs.com/
Copyright (C) 2012-present SheetJS LLC
Licensed under the Apache License, Version 2.0

SheetJS is provided via a CDN, so plugin users do not need to install a
separate library. However, if you work offline or cannot download it,
follow the steps below to download and apply it as a plugin.

1. Download dist/xlsx.full.min.js from the following website:
https://github.com/sheetjs/js-xlsx

2. Change the file name to any English characters.
(If the file name contains a ".", it will not be imported as a plugin.)

3. Enable it from the Plugin Management screen.
...
@param excelDataPath
@text Sheet data file path
@desc This is the path where the sheet data file will be output. You can enter a relative path or an absolute path.
@default excelData

@param ExportPrefix
@text Output File Prefix
@desc A prefix to be added to the export file if you want to keep it separate from the import file.

@param targetDatabase
@text Target database
@desc This is a list of databases to be input/output. You can add your own defined data files.
...
@desc The file format of the output data.
@type select
@default xlsx
...
@text Original data loading
@desc Executes the process of reading the created original JSON data. If the data file does not exist, an error will occur.
@type boolean
@default false

@param autoImport
@text Auto Import
@desc The game will start after the import process is executed. This is only valid during test play.
@type boolean
@default false

@param exportEventTest
@text Event Test Output
@desc When an event test is executed, the execution details are output. However, when the command of this plugin is executed, the execution details are not output.
@type boolean
@default true

@param originalDatabaseStack
@text Database Array
@desc When exporting and importing the original database, rows with the same ID are stored as an array.
@type boolean
@default false
