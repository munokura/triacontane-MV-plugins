...
@author triacontane
@help
During combat, a face graphic will now appear above the command selection
window.

You can also replace the face graphic with any picture image.
Enter the following in the actor's database note field:
<face_picture: (Picture filename without extension)>
...
Additionally, as an extra feature, you can change the displayed face
graphic based on the current actor's HP.
Values can be specified in 10% increments. This is achieved by creating
a separate actor in the database and writing the note field there.

<face_actor_hp80:5>
 # Apply graphic for actor ID[5] when HP is 80% or lower
<face_actor_hp30:6>
 # Apply graphics from actor ID [6] when HP is 30% or lower

You can also change the displayed face graphic when a specific state is
active. Similarly, you need to create a separate actor.

<face_actor_state5:7>
 # Apply graphics from actor ID [7] if state [5] is active
...
@desc While selecting an ally character, the currently selected target is displayed (not the target during action input).
@default ON
*/
/*:ja
