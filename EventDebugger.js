...
You can pause event execution at any point and execute it line by line
(step execution).
There are three ways to start it:

1. Press the specified key (default: F7). (*1)
2. Execute the plugin command [BREAK_POINT] (conditions can be specified).
3. Execute an event test.
4. Meet the conditions specified by the plugin command [AUTO_BREAK]. (*2)
*1 If multiple parallel events are executing, it is uncertain where
execution will stop.
*2 Step execution will begin from the next event command after the
condition is met.
If a large number of events are executing in a single frame, this feature
...
The following types of step execution are available, and you can specify
the corresponding function keys.

Step In: Executes one line. Also steps through calls to common events.
Step Over: Executes one line. All common event calls are executed at once.
Resume: Ends step execution and returns to normal event execution.

Also outputs the time it took for each event command to execute.

You can also constantly monitor the evaluation results of variables and
scripts.
When a monitored object is registered, a window will appear in the upper
right corner.
If a numeric value is specified, the variable value will be displayed, and
if a string is specified, the script evaluation result will be displayed.
You can register a monitored object by pressing the specified function key
or by running the following script
from the Developer Tools.

DebugManager.watchVariable(5);
 # Constantly monitors the value of variable [5].

This plugin only works in test mode and has no effect on the final
version.
...
BREAK_POINT \v[1] === 3
 # Starts step execution if the condition [\v[1] === 3] is met.
B \v[1] === 3
 # Same as above.
AUTO_BREAK \v[1] === 3
 # Starts step execution when the condition [\v[1] === 3] is met.
AB \v[1] === 3
 # Same as above.
