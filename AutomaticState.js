...
@plugindesc Automatic state assignment plugin
@author Triacontane
@license MIT License
...
Grant the target state while above (>=) the specified HP (percentage).
<AS下限HP: (HP percentage [percentage])>
Grant the target state while below (<=) the specified HP (percentage).
<AS上限MP: (MP percentage [percentage])>
Grant the target state while above (>=) the specified MP (percentage).
<AS下限MP: (MP percentage [percentage])>
Grant the target state while below (<=) the specified MP (percentage).
<AS上限TP: (TP Percentage)>
Applies the target state while above (>=) the specified MP (percentage).
<AS下限TP: (TP Percentage)>
Applies the target state while below (<=) the specified MP (percentage).
<AS武器装備: (Weapon ID)>
Applies the target state while the specified weapon is equipped.
<AS防具装備: (Armor ID)>
Applies the target state while the specified weapon is equipped.
<ASスイッチ: (Switch ID)>
Applies the target state while the specified switch is ON.
<AS計算式: (JS Formula)>
Applies the target state while the specified JavaScript formula is true.
<AS並び順: (Index)>
Applies the target state while the sort order (1-) matches the specified
value.

If you want to use inequality signs in your formula, use the following:
< → &lt;
> → &gt;
Example: <AS計算式:\v[2] &gt; 1> // If variable [2] is greater
than 1

2. Target of Automatic State Assignment
<ASアクター: (Actor ID)>
Sets the target of automatic state assignment to the specified actor only.
If no ID is specified, it will apply to all actors.
To specify multiple actors, separate the numbers with a comma.
Example: <ASアクター: 1, 2, 3>
...
Script
To display a specific message when an automatic state is enabled:
$gameSystem.automaticStateAddMessage = true;

To suppress a specific message when an automatic state is enabled:
