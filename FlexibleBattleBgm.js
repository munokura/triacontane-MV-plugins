...
This plugin allows you to set battle BGM for each enemy group.
This setting takes priority over the system battle BGM,
and the system battle BGM will play only if it is not set.
The setting is made using the plugin command,
and enemy group IDs can be specified all at once.
Once you run this setting, it will be saved in your save file,
so you only need to run it once at the start of the game.

Plugin Command Details
Execute from the "Plugin Command" event command.
(Separate parameters with a space.)

ADD_TROOP_BGM [Enemy Group ID Start] [Enemy Group ID End] [BGM Name] [BGM Volume] [BGM Pitch] [BGM Phase]

(Example 1) To set the battle BGM for enemy group "1" to "Battle 1"
ADD_TROOP_BGM 1 1 Battle1 100 100 0

(Example 2) To set the battle BGM for enemy groups "1" through "20" to
"Battle 2"
ADD_TROOP_BGM 1 20 Battle2
Note: If volume, pitch, and phase are omitted, they will be set to "90,"
...
(function () {
    ...
    Game_System.prototype.addTroopBattleBgm = function (start, end, name, volume, pitch, pan) {
        ...
        this._troopsBattleBgm[start + ':' + end] = {
            name: name, pan: pan.clamp(-100, 100),
