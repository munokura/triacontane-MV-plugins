...
-----
Customize critical hit chance, damage, and animation.

Enter the following in the skill's Note field.

- Apply a special formula to critical hits. The format is the same as the
damage formula.
When this formula is applied, the default 3x damage will be disabled.
<CCFormula: JavaScript Formula>
Example: <CCFormula: a.atk * 4>
 // 4x attack power, ignoring the opponent's defense

- Adds critical hit chance. The calculated result is added to the original
chance. (Percentage)
<CCProbAdd: Addition Value>
Example: <CCProbAdd:50> // Occurs at 50% higher than the original chance

- Changes critical hit chance. The original chance is ignored.(Percentage)
<CCProbChange: Change Value>
Example: <CCProbChange:\v[1]>
 // Occurs with the probability of variable [1]
...
- A dedicated message for the effects will be displayed before execution.
<CCMessage: Message Content>

* For skills that attack all enemies or multiple times, even a critical
hit will trigger the critical animation.
...
(function () {
    ...
    Window_BattleLog.prototype.showCriticalEffect = function (subject, action, targets) {
        ...
        var animationIdString = getMetaValues(subject.getData(), ['演出', 'Effect']);
        if (animationIdString) {
