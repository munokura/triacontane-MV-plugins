...
Version 3.0.0 is not compatible with previous versions.

Adds another New Game to the bottom of the title screen window.
Selecting this option will transition you to a specified map separate from
the New Game.
It can be used for a variety of purposes, including post-game bonuses, CG
flashbacks, staff credits, mini-games, and hidden elements.

Options can be hidden or disabled in advance.
These options can be disabled using the plugin command, and the disabled
state is shared throughout the game, even across save files. A disabled
state can also be disabled again.

The settings are saved in the file "AnotherNewGameMk2.rpgsave."

Plugin Command Details
Execute from the "Plugin Command" event command.

ANG_VISIBLE 1
 # Makes the [1] Another New Game visible.
ANG_ENABLE 1
 # Makes the [1] Another New Game selectable.
ANG_HIDDEN 1
 # Hides the [1] Another New Game.
ANG_DISABLE 1
 # Prevents selection of the [1]th Another New Game.

Usage Example (To display the first registered Another New Game):
ANG_VISIBLE 1
...
Please use the new game hiding function with caution, as it may prevent you
...
@param mapId
@text Map ID
@desc The map ID to move to. If set to 0, no transfer will occur.
@default 1
@type number
...
@param mapY
@text Y Coordinate
@desc The Y coordinate of the destination. (Natural number)
@default 1
@type number

@param hidden
@text Default Hidden
@desc Hides the option by default. Can be enabled via a plugin command.
@default false
@type boolean

@param disable
@text Default Disabled
@desc Disables the option by default. Can be enabled via a plugin command.
@default false
@type boolean

@param addPosition
@text Added Position
@desc The command position for Another New Game. (1: Above New Game, 2: Above Continue, 3: Above Options)
@default 0
@type select
@option Below Options
@value 0
@option Above New Game
@value 1
@option Above Continue
@value 2
@option Above Options
@value 3

@param switchId
@text Linked Switch Number
@desc Specifies the switch that will be automatically turned ON when Another New Game starts.
@default 0
@type switch

@param fileLoad
@text File Load
@desc Moves to the load screen and loads existing save data when Another New Game is selected.
@default false
@type boolean

@param noFadeout
@text Disable Fadeout
@desc Audio and screen do not fade out when Another New Game is selected.
@default false
@type boolean
*/
