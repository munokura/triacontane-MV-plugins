...
@plugindesc Dynamic Event Generation Plug-in
@author Triacontane
@license MIT License
...
Dynamically generates events by copying them.
Copied temporary events can be completely deleted using the "Temporarily
Delete Event" event command, freeing up space for objects and sprites.
Self-switches are managed individually and are initialized each time they
are generated.
...
ERS_MAKE 1 5 10
 # Copy the event with event ID [1] and place it at X[5] Y[10].

Setting the event ID to "0" will duplicate the currently running event
(this event).

●Additional Traits●
Specifying a "string" for the event ID will duplicate events with a
matching event name.
(If multiple targets exist, the smallest ID will be used.)

ERS_MAKE aaa 5 10
 # Copy the event with event name [aaa] and place it at X[5] Y[10].

The same applies to other plugin commands.

You can also randomize the generation location. This is not just random,
but is based on the following auxiliary conditions:
a. Passable (0: No judgment, 1: Passable tiles only)
b. On-screen or off-screen (0: No judgment, 1: On-screen, 2: Off-screen)
c. Overlap with other characters (0: No judgment, 1: Player, 2: Event,
 3: Both)
d. Terrain tag (0: No judgment, 1..: Specified terrain tag)
e. Region (0: No judgment, 1..: Specified region)

ERS_MAKE_RANDOM 1 1 2 3 5 4
 # Places an event with event ID [1] under the following conditions (*).
* Passable, off-screen, and no player or event present.
Randomly selects a location from among locations with a terrain tag of "5"
and a region of "4."

Multiple terrain tags and regions can be specified.
Please separate the values with commas.
ERS_MAKE_RANDOM 59 0 0 0 3,4 0
 # Placed in terrain tag [3][4]

In particular, since specifying the a, b, and c conditions was difficult,
we've made it possible to specify them in text.
The above conditions can be rewritten as follows.
(The order cannot be changed.)

ERS_MAKE_RANDOM 1 passonly offscreen both 5 4

The following are synonyms that can be used interchangeably.
none
passonly
onscreen screen
offscreen
...
You can set the number of randomly generated events. Specify this from the
seventh parameter.
ERS_MAKE_RANDOM 1 1 1 1 1 1 2
 # Randomly generate [2] events.

Finally, you can obtain the event ID of the dynamically generated event.
The specified number will be stored in the variable.

ERS_GET_LAST_SPAWN_EVENT_ID 10
 # Set the last spawned event ID to variable [10]

Setting the event ID to "0" will duplicate the currently running event
(this event).

- Integration with Other Plugins
When combined with the template event plugin "TemplateEvent.js,"
Events defined in a template map can be generated directly into the map.
Executing the command without applying "TemplateEvent.js" will result in
an error.

ERS_MAKE_TEMPLATE 1 5 10
 # Generates the event ID [1] for the template map.

ERS_MAKE_TEMPLATE_RANDOM 1 1 2 3 5 4
 # Randomly generate template [1]
* Supporting conditions are the same as "ERS_MAKE_RANDOM".

Source: "TemplateEvent.js"
https://raw.githubusercontent.com/munokura/triacontane-MV-plugins/refs/heads/main/TemplateEvent.js

When using in conjunction with YEP_EventMiniLabel.js, place this plugin
below.

- About the Deterministic Output Method
When determining the event generation location, it will generate at a
location that meets the conditions, rather than randomly trying them.
During generation, the entire map is scanned to find a location that meets
the conditions.
This is effective when there are few candidate locations, such as when
specifying only a region.
When the "Deterministic Output Method Switch" parameter is ON, output will
be in this mode.

Furthermore, you can place events and template events simply by placing
regions on the map. Please enter a tag in the Note field of the source
event.

- Place a copy of the event in the square where Region [1] is placed.
<CP:1>
...
@desc If enabled, the self-switch will not be cleared when moving locations. It will be cleared when clearing events is executed.
@type boolean
@default false

@param tryRandomCount
@text Number of attempts
@desc The number of attempts to generate a random event. If the event generation fails, increase the value.
@type number
@default 1000

@param CertainSpawnSwitch
@text Fixed output method switch
@desc When the specified switch is ON, the "Determined Output Method" will be used during generation. Please check the help for details.
@type switch
@default 0
*/
