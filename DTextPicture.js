...
This command dynamically generates a picture from a specified string.
The string can contain various control characters (such as \v[n]),
allowing the picture to update in real time when the value of the variable
displayed with the control characters changes.

To display a picture, follow these steps:
1: Specify the string and arguments you want to draw using the [D_TEXT]
plugin command (see example below).
2: Specify the background color and alignment using the [D_TEXT_SETTING]
plugin command (optional).
3: Deselect "Image" in the "Show Picture" event command.
* The picture will not be displayed in step 1, so be sure to call both
commands as a set.
* You can display multiple lines by running D_TEXT multiple times before
displaying the picture.

* As of version 1.4.0, if you specify "Image" in "Show Picture" after
running [D_TEXT], the dynamic string picture generation will be postponed
and the "Image" picture will be displayed as usual.

Plugin Command Details
Execute from the "Plugin Command" event command.
(Arguments must be separated by a space.)

D_TEXT [Draw String] [Character Size]: Prepares for dynamic string picture
generation.
Example: D_TEXT TestString 32

Once displayed, it can be moved, rotated, and erased just like a normal
picture.
It also supports control characters such as those used to display
variables and actors.

D_TEXT_SETTING ALIGN [Alignment]:Sets the alignment (left, center, right).
0: Left, 1: Center, 2: Right

Example:
D_TEXT_SETTING ALIGN 0
D_TEXT_SETTING ALIGN CENTER

Note: When multiple lines are specified, the alignment setting is aligned
to the widest line.
Therefore, this setting does not work when drawing only a single line.

D_TEXT_SETTING BG_COLOR [background color]: Sets the background color
 (same format as CSS colors).

Example:
D_TEXT_SETTING BG_COLOR black
D_TEXT_SETTING BG_COLOR #336699
D_TEXT_SETTING BG_COLOR rgba(255,255,255,0.5)

Specifies the background color gradient in pixels.
D_TEXT_SETTING BG_GRADATION_RIGHT [pixels]
D_TEXT_SETTING BG_GRADATION_LEFT [pixels]

Example:
D_TEXT_SETTING BG_GRADATION_RIGHT 50
D_TEXT_SETTING BG_GRADATION_LEFT 50

D_TEXT_SETTING REAL_TIME ON: Real-time display of variables displayed with
control characters

Example:
D_TEXT_SETTING REAL_TIME ON

When real-time display is enabled, the picture content is automatically
updated when the variable value changes after the picture is displayed.

D_TEXT_SETTING WINDOW ON: Displays a window in the background.
Example:
D_TEXT_SETTING WINDOW ON

D_TEXT_SETTING FONT [font name]: Changes the font used in drawing to the
specified name.
Example:
D_TEXT_SETTING FONT MS P Mincho

To safely use the font change function, you will need the separately
released "Font Load Plugin."

As with D_TEXT, these settings should be made before displaying the
picture.

Supported Control Characters (same as the "Show Text" event command)
\V[n]
\N[n]
\P[n]
\G
...
\f[b] Bold font
\f[i] Italicize font
\f[n] Return bold and italic fonts to normal

*1 How to specify outline color
\oc[red] Specify by color name
\oc[rgb(0,255,0)] Specify by color code
\oc[2] Specify by text color number (same as \c[n])

Displays the cursor when a window is displayed in the background.
Execute this command after displaying a dynamic string picture.
D_TEXT_WINDOW_CURSOR 1 ON # Display the window cursor in picture[1]
D_TEXT_WINDOW_CURSOR 2 OFF # Remove the window cursor in picture[2]

The following commands change the active cursor state.
D_TEXT_WINDOW_CURSOR_ACTIVE 2 ON # Activate the cursor in Picture [1]
D_TEXT_WINDOW_CURSOR_ACTIVE 1 OFF # Stop the cursor in Picture [1]

To specify the coordinates of the cursor rectangle, use the following:
D_TEXT_WINDOW_CURSOR 1 ON 0 0 100 100 # Display a window cursor with
dimensions [0,0,100,100] in Picture [1]
...
@desc When the switch with the specified number is ON, the icon will be displayed with \ITEM[n]. If not specified, it will always be displayed.
@type switch
@default 0

@param lineSpacingVariableId
@text Line spacing correction variable ID
@desc When displaying multiple lines, the line spacing will be adjusted by the value of the specified variable. If you set a value that is too large, some text may be cut off.
@type variable
@default 0
