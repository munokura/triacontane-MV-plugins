...
-----
Set application conditions for each effect.
Effects that do not meet the conditions will be disabled.
Enter the following in the database Note field where you describe the
effect.

<EC1Switch:10>
 // When switch [10] is ON, the first effect is enabled.
<EC1Cond:JSExpression>
 // When the JS expression evaluates to true, the first effect is enabled.
< → &lt;
> → &gt;
Example:
 <EC1Cond:\v[1] &gt; 10>
...
You can refer to the caster by writing "subject" in the script.
You can refer to the target by writing "target" in the script. (*1)
You can refer to the skill's effect amount by writing "damage" in the
script. (*2)

*1 However, this cannot be used if the effect type is "Common Event."
*2 Recovery will result in a negative value.

Example: <EC1Cond: target.hpRate() === 1>
 # Effective only if the opponent's HP is at maximum.

The second and subsequent effects can be set in the same way.
