...
@plugindesc Comprehensive development support plugin
@author Triacontane
@license MIT License
...
This is a development support plugin that adjusts the behavior of the
developer tools.
This plugin is only active during test play in a local environment.
It also outputs useful information to the log, such as the UserAgent and
core script version.
The following features are provided to support smooth development:

1. The developer tools launch automatically when the game starts.
(Usually launched with F8).
Even if disabled, they will launch automatically if an error occurs.

2. They keep the game screen in the foreground. This is convenient for
working while looking at the screen. You can switch between them from the
menu bar during gameplay.

3. If you modify a map or event and resave it, the map and database will
automatically reload the moment you return focus to the game screen.

4. You can skip the title screen and load the latest save file.

5. You can speed up the game on the map (up to 16x speed).
(Fast speed, fade-out, and fast message skip on the map).
Conversely, you can also slow it down (up to 1/60x speed). Set a negative
value for the speed multiplier. The BGM pitch will change when the game
speed changes.

6. You can force annihilate all enemies to win. You can also obtain
rewards. Force defeat and abort are also possible.

7. Execute a script every frame.
The script's return value is output to the console only when it changes.

8. Temporarily stop game updates and freeze the screen.

9. Save data can be saved and loaded in JSON format without encoding.

This plugin does not have any plugin commands.

When integrated with the Screen Capture Management plugin, a menu bar for
screen captures will be displayed.
https://raw.githubusercontent.com/triacontane/RPGMakerMV/master/MakeScreenCapture.js

・Advanced Settings
If you want to enable the plugin's debug function even when launching in a
browser, add "test" or "best" to the launch URL option.
However, some NW.js-derived features will be disabled.

Normally, battle tests cannot be performed when launching the game
externally, such as from a browser, but with this plugin enabled, battle
tests are possible.

When launching from an external editor, the following launch parameters
can be specified.
These parameters will override the plugin parameters.
devToolOff: Prevents the developer tools from launching.
onTop: Displays the screen in the foreground.
...
@param FuncKeyReload
@desc These keys (F1 to F12) reload the screen. They have the same function as the default F5 key.
@type select
@default F12
@option none
@option F1
...
@param FuncKeyOnTop
@desc These keys switch the game screen to the foreground (F1 to F12).
@type select
@default F11
@option none
@option F1
...
@desc These keys switch the speed of the game (F1 to F12).
@type select
@option none
@option F1
@option F2
...
@desc These keys (F1 to F12) will force the battle to end and count it as a victory.
@type select
@option none
@option F1
@option F2
...
@desc These keys will force the battle to end as a defeat (F1 to F12).
@type select
@option none
@option F1
@option F2
...
@desc These keys (F1 to F12) will interrupt the battle and force it to end.
@type select
@option none
@option F1
@option F2
...
@param FuncKeyScript
@desc These are keys for running scripts (F1 to F12).
@type select
@option none
@option F1
@option F2
...
@param FuncKeyFreeze
@desc Keys to pause screen updates (F1 to F12).
@type select
@default F10
@option none
@option F1
...
@param CutTitle
@desc Skip the title screen and load the most recent save file.
@type boolean
@default false
...
@param RapidSpeed
@desc The speed multiplier when speeding up is performed. (-60...16)
@type number
@default 2
@min -60
@max 16
...
@param JsonSave
@desc You can save and load in JSON format. You can freely edit the save file using a text editor.
@type boolean
@default false

@param OutputStartupInfo
@desc Logs various information at startup.
@type boolean
@default true

@param StartupOnTop
@desc The game screen will be fixed to the foreground when launched.
@type boolean
@default false
...
@desc When the game screen loses focus while it is displayed in the foreground, the screen will be shifted to the right by the specified coordinates.
@type number
@default 640

@param GreetingHide
@desc Hides the greeting message on the title screen.
@type boolean
@default false

@param BirthdayMonth
@desc If you enter your birthday, a message will appear on the title screen on that day.
@type number
@default 0
@min 0
@max 12
