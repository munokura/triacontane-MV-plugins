...
In the "Item Selection" event command,
You can set display conditions for each item.
Set the following in the Note field for the target item.

<EICSwitch: 1> # Display only when switch [1] is ON
<EICScript: s> # Display only when the evaluation result is true
If you want to use inequality signs in your script, write them as follows:
< → &lt;
> → &gt;
...
Instead of "hiding items that do not meet the conditions," you can also
change the text color of items that meet the conditions.
To do so, specify the "Text Color Number" parameter.
...
@desc Whether to display items that do not have a specified Note field. If you turn it off, items that do not have a specified Note field will not be displayed.
@type boolean
@default true
...
@desc If you specify this, the text color will change when the condition is met. (If the condition is not met, the text will be displayed normally.)
@type number
@default 0
@min 0
@max 20
