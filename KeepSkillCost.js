...
@plugindesc Skill Maintenance Cost Plugin
@author Triacontane
@license MIT License
...
You can create skills that consume a set cost after a turn ends,
regardless of whether the skill is used or not, simply by possessing them.

You can use the control character \v[n] for the value.
A negative value will restore HP.

Specify the following in the skill's Note field.

<KSC_HP:10>
- Consumes 10 HP after the turn ends. (Treats as slip damage)

<KSC_MP:10>
- Consumes 10 MP after the turn ends.

<KSC_TP:10>
- Consumes 10 TP after the turn ends.

<KSC_Gold:10>
- Consumes 10 Money after the turn ends. (Only valid for actors)

<KSC_VariableNumber:5>
- Subtracts [10] from the value of variable [5] after the turn ends.

<KSC_VariableValue:10>
- Used in conjunction with the above.

When combined with YEP_SkillCore.js, maintenance costs will be displayed in
the skill window. Please place this plugin below.
However, the only maintenance costs output will be HP, MP, and TP.
...
@param BattleOnly
@desc Consumption at the end of a turn is only enabled on the battle screen.
@default ON
*/
