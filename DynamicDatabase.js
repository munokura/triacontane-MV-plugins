...
This plugin changes database fields to dynamic values.
You can build more advanced databases using variables and JavaScript
functions.

Caution! Do not use in conjunction with plugins that save or update
database files.

String fields: You can use control characters entered directly into each
field.
Number fields: You can use control characters entered in the Note field
plus JavaScript formulas.
Selection fields: You can use control characters entered in the Note field
plus JavaScript formulas.
*The top selection will be 0.

For number and selection fields, please specify the following in the memo
field:
<DD(item name): (formula)>

Example 1
(Setting the item price to twice the value of variable number 10)
 <DD価格:\V[10] * 2>

Example 2
(Setting the weapon's attack power to the product of the original value and the value of variable number 10)
 <DD攻撃力:prev * \V[10]>

If you want to use an inequality sign in a formula, please write it as
follows:
< → &lt;
> → &gt;

If you want to dynamically create a database of traits and effects,
define the actual traits and effects in addition to the Note field.
(The Note field takes priority, so only the frame is required.)
...
For details on the configurable items, see the following link:
